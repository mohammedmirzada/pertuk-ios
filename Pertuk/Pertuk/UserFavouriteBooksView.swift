...
class FetchDataTwo: ObservableObject{
    var isHidden = false {
        willSet {
    ...
    init() {
        let url = URL(string: "https://programnas.com/control/template/php/api/library/get/?API=\(user.API_AUTH)&tool=rf&get=favorite&user_id=\(user.GetUserId())")
...
struct UserFavouriteBooksView: View {
    @Environment(\.colorScheme) var colorScheme
    @StateObject var fetchDataTwo = FetchDataTwo()
    @EnvironmentObject var adsViewModel: AdsViewModel
    
    var body: some View {
        NavigationStack{
            if fetchDataTwo.isHidden{
                if fetchDataTwo.found{
                    ScrollView{
                        VStack{
                            Spacer()
                            VStack{
                                LazyVGrid(columns: UIDevice.current.userInterfaceIdiom == .pad ? [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())] : [GridItem(.flexible()), GridItem(.flexible())], spacing: 20) {
                                    ForEach(self.fetchDataTwo.book_ids, id: \.self) { book_id in
                    ...
                    }.navigationTitle("my_read_favourite").toolbarRole(.editor)
                }else{
                    VStack{
...
struct UserFavouriteBooksView_Previews: PreviewProvider {
